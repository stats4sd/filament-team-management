...
it('does not save an unchanged latest tenant or a wrong-model tenant', function () {
    $user = User::factory()->create();
    $team = Team::factory()->create();
    $user->latestTeam()->associate($team)->save();
    $this->actingAs($user);
    Filament::setTenant($team, isQuiet: true);
    $writes = 0;
    User::saving(function () use (&$writes) {
        $writes++;
    });
    (new SetLatestTeamMiddleware)->handle(new Request, passThrough());
    Filament::setTenant($user, isQuiet: true);
    (new SetLatestTeamMiddleware)->handle(new Request, passThrough());
    expect($writes)->toBe(0)->and($user->fresh()->latest_team_id)->toBe($team->getKey());
});
