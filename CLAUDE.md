...
- `Support\MembershipMutation` coordinates bulk/creation/removal/lifecycle writes with user-first locks and current pivot graph validation for deletion; invitation actions validate token capabilities separately. Host participants run before change and after write inside the transaction. Observation events/mail run after successful outer commit.
- `Support\MembershipCandidates` owns explicit-actor candidate discovery and complete submitted-ID validation; membership batches still authorize each mutation. `Actions\CreateTeamForProgram` owns atomic creation/linking.
- `Filament\Support` holds shared member/invitation UI, selector search/labels/policy filtering and navigation. Profile viewing, editing, member viewing and leaving are separate abilities. Retain server-side checks on direct widget/relation-manager requests.
